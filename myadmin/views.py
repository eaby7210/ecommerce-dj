...
class Dashboard(APIView):
    ...
    def generate_pdf(self, context):
        ...
        title_paragraph = Paragraph(
            f"{title}<br/>Report - {generation_date}", styles['Title'])
        elements.append(title_paragraph)
        elements.append(Spacer(1, 0.5 * inch))

        # Membership counts table
        membership_data = [['Membership Type', 'Number of Customers']] + [
            [membership['membership_display'], membership['count']] for membership in context['membership_counts']]
        membership_table = Table(membership_data, hAlign='LEFT')
        membership_table.setStyle(self.get_table_style())
        elements.append(
            Paragraph("Number of customers per membership type", styles['Heading2']))
        elements.append(membership_table)
        elements.append(Spacer(1, 0.5 * inch))

        # Order status table
        status_data = [['Order Status', 'Number of Orders']] + \
            [[status['order_status_display'], status['count']]
                for status in context['status_counts']]
        status_table = Table(status_data, hAlign='LEFT')
        status_table.setStyle(self.get_table_style())
        elements.append(
            Paragraph("Orders broken down by status", styles['Heading2']))
        ...
        payment_method_data = [['Payment Method', 'Number of Orders']] + [
            [payment['payment_method_display'], payment['count']] for payment in context['payment_method_counts']]
        payment_method_table = Table(payment_method_data, hAlign='LEFT')
        payment_method_table.setStyle(self.get_table_style())
        elements.append(
            Paragraph("Payment method distribution", styles['Heading2']))
        ...
        orders_per_customer_data = [['Customer', 'Number of Orders']] + [
            [order['customer'], order['count']] for order in context['orders_per_customer']]
        orders_per_customer_table = Table(
            orders_per_customer_data, hAlign='LEFT')
        ...
        order_item_status_data = [['Order Item Status', 'Number of Items']] + [
            [status['status_display'], status['count']] for status in context['order_item_status_counts']]
        order_item_status_table = Table(order_item_status_data, hAlign='LEFT')
        order_item_status_table.setStyle(self.get_table_style())
        elements.append(
            Paragraph("Status distribution of order items", styles['Heading2']))
        elements.append(order_item_status_table)
        elements.append(Spacer(1, 0.5 * inch))

        # Most ordered products table
        most_ordered_products_data = [['Id', 'Product Title', 'Number of Orders']] + [[product['product__id'],
                                                                                       product['product__title'], product['count']] for product in context['most_ordered_products']]
        most_ordered_products_table = Table(
            most_ordered_products_data, hAlign='LEFT')
        most_ordered_products_table.setStyle(self.get_table_style())
        elements.append(
            Paragraph("Most frequently ordered products", styles['Heading2']))
        elements.append(most_ordered_products_table)
        elements.append(Spacer(1, 0.5 * inch))

        # Top brands table
        top_brands_data = [['Brand', 'Number of Orders']] + [
            [brand['product__brand__title'], brand['total_orders']] for brand in context['top_brands']]
        ...
        top_categories_data = [['Category', 'Number of Orders']] + [
            [category['product__category__title'], category['total_orders']] for category in context['top_categories']]
    ...
    def get(self, request):
        ...
        order_years = orders.annotate(year=ExtractYear("placed_at")).values(
            "year").annotate(count=Count("id")).values("year", "count")
        order_months = orders.annotate(month=ExtractMonth("placed_at")).values(
            "month").annotate(count=Count("id")).values("month", "count").order_by("month")
        ...
        membership_display_case = Case(
            *[When(membership=choice[0], then=Value(choice[1]))
              for choice in Customer.MEMBERSHIP_CHOICES],
        ...
        order_status_display_case = Case(
            *[When(order_status=choice[0], then=Value(choice[1]))
        ...
        payment_method_display_case = Case(
            *[When(payment_method=choice[0], then=Value(choice[1]))
        ...
        # Coupon usage
        orders_with_coupons = orders.filter(
            applied_coupon__isnull=False).count()
        orders_without_coupons = orders.filter(
            applied_coupon__isnull=True).count()

        # Orders per customer
        orders_per_customer = orders.values(
            'customer').annotate(count=Count('id'))
        ...
        most_ordered_products = OrderItem.objects.values(
            'product__id', 'product__title').annotate(count=Count('id')).order_by('-count')[:10]
...
class UserViewSet(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        user_data = {key.split(".")[1]: value for key, value in request.data.items(
        ) if key.startswith("user.")}
        customer_data = {key: value for key, value in request.data.items(
        ...
        if user_serializer.is_valid():
            serializer = CustomerUpdateSerializer(
                instance, data=customer_data, partial=True)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            errors = [f"{field} =>{error[0]}" for field,
                      error in serializer.errors.items()]
...
class ProductViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(
                context, template_name="admin/products_add_form.html", content_type='text/html'
            )
...
class CategoryAdminViewSet(ModelViewSet):
    queryset = Main_Category.objects.annotate(
        products_count=Count('categories_products')).all()
...
class OrderViewSet(ModelViewSet):
    ...
    queryset = Order.objects.select_related(
        'customer', 'address', 'applied_coupon').prefetch_related('items').all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if data['valid_to'] <= data['valid_from']:
                messages.error(
                    request, "Valid_form cannot be greated than valid_to")
                context = {
                    'serializer': serializer
                }
...
class SalesReportView(APIView):
    ...
    def get_context_data(self, orders, date_range, start_date, end_date):
        ...
        total_orders = len(orders) if isinstance(
            orders, list) else orders.count()
        return {
            'orders': orders,
    ...
    def generate_pdf(self, context):
        ...
        p.drawString(100, height - 140,
                     f"Report: {context['date_range'].title()}")

        # Total sales and orders
        p.drawString(100, height - 160,
                     f"Total Sales: {context['total_sales']}")
        p.drawString(100, height - 180,
        ...
        if not request.htmx:
            ...
            if 'export' in request.data:
                ...
                response = paginator.get_paginated_response(
                    paginated_orders, date_range, start_date, end_date)
                if response == None:
    ...
    def post(self, request):
        ...
        context = self.get_context_data(
            orders, date_range, start_date, end_date)
        return self.export_to_pdf(context)
