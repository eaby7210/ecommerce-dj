    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                email_otp(user)
                messages.success(
                    request, "Account created successfully! An OTP is sent to your Email")
                return redirect("verify-email", user_id=user.id)
                authuser = auth.authenticate(
                    request, username=serializer.validated_data["username"], password=serializer.validated_data["password1"])
            ...
            if bool(user) and bool(authuser):
                ...
                messages.error(
                    request, 'Error while authentication, try login again')
            ...
            if email_otp.expires_at > timezone.now():
                if otp == email_otp.otp:
                    ...
                    if request.user.is_authenticated:
                        ...
                        messages.success(
                            request, "Email verified successfully you can now login again")
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if bool(user and user.is_staff):
                ...
                messages.info(
                    request, message='Logged in as admin succesfully')
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if user is None or not user.is_active:
                ...
                customer, created = Customer.objects.prefetch_related(
                    'user').get_or_create(user_id=user.id)
                if created:
                    messages.info(
                        request, f"{user.username}'s profile is created as Bronze Membership")
    ...
    def put(self, request):
        ...
        if mode == "chnge_pass":
            data = {key: value for key, value in request.data.items()
                    if key != "mode"}
            pass_serializer = ChangePasswordSerializer(
            ...
            if pass_serializer.is_valid():
                ...
                messages.info(
                    request, "You will be redicted to login Page to login again")
            ...
            user_data = {key: value for key, value in request.data.items() if bool(
                key != "birth_date" and key != "mode")}
            customer_data = {key: value for key, value in request.data.items() if bool(
                key == "birth_date" and key != "mode")}
            user_serializer = UserNormalUpdateSerializer(
                customer.user, data=user_data, partial=True)
            customer_serializer = CustomerProfiledSerializer(
...
class AddressViewSet(ModelViewSet, AdressPagination):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            if len(serializer.data) <= 0:
                messages.info(
                    request, "There is no saved Address.\
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if mode == 'checkout-primary':
            try:
                instance = Address.objects.get(
                    primary=True, customer=self.get_customer_id().id)
            except Address.DoesNotExist:
                messages.warning(
                    request, "There is no primary saved address in your\
                        profile, Please add or select any other address")
                return Response(
                    {'address': None},
                    template_name="app/checkout-address.html",
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            instance, message = serializer.update(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.primary == True:
            ...
            address.save()
            messages.info(
                request, f"{address.name} has set to primary address")
