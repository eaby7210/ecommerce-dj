    ...
    def get_queryset(self):
        if self.request.user.is_authenticated:
            ...
            return Product.objects.annotate(
                ...
                Prefetch(
                    'cartitem_set',
                    queryset=CartItem.objects.only(
                        'id', 'quantity').filter(customer=customer),
                    to_attr='cart_items'
                ),
                Prefetch(
                    'wish_product',
                    queryset=WishList.objects.only(
                        'id').filter(customer=customer),
...
class BrandViewSet(ReadOnlyModelViewSet):
    queryset = Brand.objects.annotate(
        products_count=Count('brand_products')).filter(active=True)
...
class CategoryViewSet(ReadOnlyModelViewSet):
    queryset = Main_Category.objects.annotate(
        products_count=Count('categories_products')).filter(active=True)
    pagination_class = CustomPagination
...
class CartViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        if CartItem.objects.filter(customer=self.get_customer_id()).count() == 0:
            messages.error(
                request, "Cart is Empty,Please add products to view the cart.")
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if page == 'list':
                ...
                response = Response(
                    context, template_name="app/cart-add-listpage.html", content_type="text/html")
                return response
            elif page == 'detail':
                context = {
                    "item": cart
                ...
                response = Response(
                    context, template_name="app/cart-add-detailpage.html", content_type="text/html")
                return response
            elif page == 'wishlist':
                if instance == None:
    ...
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        messages.warning(
            request, f"{instance.product.title} has been removed from cart")
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if mode == "pdfgen":
            ...
            title_paragraph = Paragraph(
                f"{title}<br/>Date: {generation_date}", getSampleStyleSheet()['Title'])
            ...
            elements.append(
                Paragraph("Order Items", getSampleStyleSheet()['Heading2']))
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            if payment_method == "cod":
                message, order, total = serializer.save(
                    coupon=coupon, payment_method=payment_method)
                if order == None:
                ...
                message, razorpay_obj, ptotal = serializer.save(
                    coupon=coupon, payment_method=payment_method)
                if razorpay_obj == None:
                    messages.error(request, message)
                    return redirect(reverse('checkout'))
                ...
                response = self.razor_pay_reponse(
                    razorpay_obj, message, ptotal)
    ...
    def update(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            if mode == 'retry_rzr':
                ...
                razorpay_obj = RazorpayOrders.objects.select_related(
                    'order').get(order_id=pk)
                message = "Please complete your payment through RazorPay gateway"
                response = self.razor_pay_reponse(
                    razorpay_obj, message, razorpay_obj.order.grand_total)
                ...
                if instance.payment_status == 'C':
                    ...
                    transaction_obj = Transaction.objects.create(
                        customer=customer, order=instance, amount=instance.payed_total, transaction_type='refund')
                ...
                try:
                    ...
                    messages.warning(
                        request, f"Requested for return on item {str(item)}")
                except Exception as e:
                    messages.error(
                        request, f"Could not process return for item {itemid}", e)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not (coupon.valid_from <= today <= coupon.valid_to):
                messages.warning(
                    request, "This coupon is expired or not yet valid.")
...
class CheckoutView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        cartitems = CartItem.objects.select_related(
            'product').filter(customer=self.get_customer())
        ...
        for item in cartitems:
            if item.product.inventory == 0 or item.quantity > item.product.inventory:
                messages.warning(
                    request, "There are Products that is Out of stock or more than that of stock")
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            try:
                ...
                rzr = RazorpayOrders.objects.prefetch_related(
                    'order').get(id=razorpay_order_id)
                ...
                if rzr.order.payment_status == 'C':
                    ...
                    messages.warning(
                        request, "Order payment seems to be already Completed. Amount will be credited to your wallet.")
                ...
                transaction_obj = Transaction.objects.create(
                    customer=customer, order=rzr.order, amount=rzr.order.grand_total, transaction_type='order')
                rzr.order.save()
                rzr.save()
                return redirect(reverse('u-order-detail', args=[rzr.order.id]))
            except:
